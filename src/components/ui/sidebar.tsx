...
function SidebarProvider({
  ...
  const onToggleShortcut = React.useEffectEvent((event: KeyboardEvent) => {
    if (
...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      {/* transform-gpu: the app shell never scrolls, so Chrome doesn't
          promote this fixed container on its own. Painted into the page
          layer, the strip it vacates while collapsing sometimes kept stale
          label pixels; on its own layer that strip is just uncovered. */}
      <div
      ...
      >
        <div
          ...
          className="flex h-full w-full flex-col whitespace-nowrap bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
